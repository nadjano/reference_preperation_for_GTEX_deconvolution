...
get_300_cells_per_celltype <- function(seurat){
  ...
  for (i in 1:length(unique(seurat$cellType))){
    if (unique(seurat$cellType[i] == 'neuron')){
      n = 3000
    }
    else if (length(rownames(cellSplits[[i]])) < n){
...
seurat = readRDS('sc_SUBSTANTIANIGRA/Midbrain (M) - Substantia Nigra - SN.rds')
data
GetAssay(seurat, assay = "RNA")
seurat@assays$RNA


pbmc <- SetAssayData(object = seurat, slot = "counts", new.data = data)
...
DimPlot(pbmc, label = TRUE)   + NoLegend() +ggtitle('substantian nigra')
...
saveRDS(pbmc, 'sc_SUBSTANTIANIGRA/E-MTAB-5214_tabula_substantianigra_seurat.rds')
count(pbmc$cellType)
library(plyr)
...
sc = readRDS('sc_SUBSTANTIANIGRA/E-MTAB-5214_tabula_substantianigra_seurat.rds')
